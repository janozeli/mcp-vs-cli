...
- Layer 4: the aggregate from dsh's own accounting, never a number remembered from a terminal

## Process

One transformation: a measurement becomes a claim, with its confidence attached.

1. Recompute from the traces. If dsh's own accounting cannot produce the number, it is not
