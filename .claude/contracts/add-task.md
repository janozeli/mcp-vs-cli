...
- Layer 3: `src/tasks.ts` — the `Task` shape, the difficulty gradient, the grading rules
- Layer 3: `src/tasks.test.ts` — the hand-written API the suite tests solvers against
...
2. The `answer` field is a drift reference, not the grading key. Record what the API said when the
   task was written, and let `bun run ground-truth` tell you when that stops being true.
...
6. Declare `requires`. It feeds `sample(registry, n, { keep })`, and without it an N-scaling run can
...
- a `Task` in `src/tasks.ts`, with its solver
- a fixture for it in `src/tasks.test.ts`, hand-written and deliberately not the real values
- tests alongside `src/tasks.test.ts`, exercising the solver's logic against that fixture
...
- `bun run ground-truth` agrees with the recorded reference
