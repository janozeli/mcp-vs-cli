...
def cassette() -> Cassette:
    def refuse(request: httpx.Request) -> httpx.Response:
        raise AssertionError(f"filtering must replay, but tried to reach {request.url}")
...
def test_both_formats_filter_to_the_same_answer(registry: spec.Registry, cassette: Cassette) -> None:
    from_mcp = execute.call_operation(
        registry, cassette, "list_votacoes_votos", {"id": VOTES, "_jq": COUNT_SP_YES}
    )
    from_cli = execute.run_command(
        registry, cassette, f"api list_votacoes_votos {VOTES} | jq '{COUNT_SP_YES}'", allow_pipe=True
    )
    assert from_mcp.text == from_cli.text == "22"


def test_filtering_is_what_makes_the_payload_affordable(registry: spec.Registry, cassette: Cassette) -> None:
    whole = execute.call_operation(registry, cassette, "list_votacoes_votos", {"id": VOTES})
    filtered = execute.call_operation(
        registry, cassette, "list_votacoes_votos", {"id": VOTES, "_jq": COUNT_SP_YES}
    )
    # The point of the axis: the same answer, at a fraction of the context.
    assert tokens.count_text(whole.text) > 100 * tokens.count_text(filtered.text)


def test_a_bad_expression_comes_back_as_an_error(registry: spec.Registry, cassette: Cassette) -> None:
    result = execute.call_operation(
        registry, cassette, "list_votacoes_votos", {"id": VOTES, "_jq": "this is not jq"}
    )
...
def test_pipes_are_refused_when_the_cell_does_not_have_them(
    registry: spec.Registry, cassette: Cassette
) -> None:
    result = execute.run_command(registry, cassette, f"api list_votacoes_votos {VOTES} | jq '.'")
    assert not result.ok
    assert "pipes are not available" in result.text


def test_only_jq_may_follow_the_pipe(registry: spec.Registry, cassette: Cassette) -> None:
    for command, expected in (
        ...
        result = execute.run_command(registry, cassette, command, allow_pipe=True)
        assert not result.ok, command
        assert expected in result.text, command


def test_help_still_works_through_a_filtering_cell(registry: spec.Registry, cassette: Cassette) -> None:
    result = execute.run_command(registry, cassette, "api --help", allow_pipe=True)
...
def test_the_filter_parameter_never_reaches_the_api(registry: spec.Registry, cassette: Cassette) -> None:
    # `_jq` is applied after the response arrives; sending it upstream would change the request and
    # fork the cassette.
    result = execute.call_operation(
        registry, cassette, "list_deputados", {"siglaUf": "AC", "itens": 100, "_jq": ".dados|length"}
    )
